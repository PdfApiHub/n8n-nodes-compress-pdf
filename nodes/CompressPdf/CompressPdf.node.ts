...
import { description, execute } from './actions/compressPdf';
...
export class CompressPdf implements INodeType {
	description: INodeTypeDescription = {
		...
		group: ['transform'],
		version: 1,
		description: 'Reduce PDF file size without losing quality using PDF API Hub',
		defaults: { name: 'Compress PDF' },
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		for (let i = 0; i < items.length; i++) {
			try {
				await (execute as Function).call(this, i, returnData, 'compressPdf');
			} catch (error) {
